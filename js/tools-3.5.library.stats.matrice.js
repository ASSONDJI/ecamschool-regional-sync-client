tools.Library.Stats = {
    ...
    Matrice: function () {
        ...
        /**
         * toTree()
         * Rôle : transformer la matrice en arbre hiérarchique.
         *        Chaque colonne de colsNames devient un niveau de
         *        profondeur dans l'arbre, dans l'ordre donné.
         *
         * Paramètre :
         *   - colsNames : tableau ordonné des colonnes servant de niveaux
         *                 ex: ["regime", "idClasse"]
         *
         * Retour :
         *   Un objet racine de la forme :
         *   {
         *     value    : null,          // la racine n'a pas de valeur
         *     rows     : [...],         // toutes les lignes de la matrice
         *     children : { ... }        // les nœuds du premier niveau
         *   }
         */
        this.toTree = function (colsNames) {

            // La racine contient TOUTES les lignes au départ
            let racine = {
                value: null,
                rows: this.getLignes().slice(), // copie pour ne pas modifier l'original
                children: {}
            };

            // Construction récursive niveau par niveau
            this._buildTreeLevel(racine, colsNames, 0);
        ...
        /**
         * _buildTreeLevel()
         * Rôle : fonction interne récursive qui construit un niveau
         *        de l'arbre à partir d'un nœud parent et d'une liste
         *        de colonnes restantes.
         *
         * Paramètres :
         *   - node       : le nœud parent à enrichir avec des children
         *   - colsNames  : la liste complète des colonnes (niveaux)
         *   - levelIndex : l'index du niveau courant dans colsNames
         */
        this._buildTreeLevel = function (node, colsNames, levelIndex) {

            // Cas d'arrêt : plus de colonnes à traiter → c'est une feuille
            if (levelIndex >= colsNames.length) {
                return;
            }

            let colName = colsNames[levelIndex];
            let indexCol = this.getColIndex(colName);

            // Regrouper les lignes du nœud courant par valeur de cette colonne
            // groupedByValue : { "valeur1": [ligneName1, ligneName2], "valeur2": [...] }
            let groupedByValue = {};

            for (let i = 0; i < node.rows.length; i++) {
                let ligneName = node.rows[i];
                let val = this.getElement(colName, ligneName);

                // Normaliser en string pour servir de clé d'objet stable
                let key = (val === null || val === undefined) ? "__NULL__" : String(val);

                if (groupedByValue[key] === undefined) {
                    groupedByValue[key] = { value: val, rows: [] };
                }
                groupedByValue[key].rows.push(ligneName);
            }

            // Créer un nœud enfant pour chaque valeur distincte trouvée
            for (let key in groupedByValue) {
                if (Object.hasOwnProperty.call(groupedByValue, key)) {
                    let childNode = {
                        value: groupedByValue[key].value,
                        rows: groupedByValue[key].rows,
                        children: {}
                    };

                    node.children[key] = childNode;

                    // Descendre récursivement au niveau suivant pour ce nœud
                    this._buildTreeLevel(childNode, colsNames, levelIndex + 1);
                }
            }
        };
        ...
        this.addColElement = function (val, colName) {
            let lignName = "index" + this.lignes.length;
            let elementExist = this.elementExist(colName, lignName);
            // currentIndexLignCol
            if (!elementExist) {
                this.setElement(val, colName, lignName);
            } else {
                this.setElement(val, colName, "index" + (this.lignes.length + 1));
        ...
        this.getConfigGroupExport = function (config, colonnesShow, title = "Label Config") {
            let
                ...
                points = this._getPointsWithColValidators(config["config"]["validators"]);
            // resume = this._iteratorAccumulator(points, config["config"]["accumulator"]);
            // console.log(resume);
            // console.log(colonnesShow);
            ws_data.push(["LISTE"]);
        ...
        this.getConfigGroupResume = function (configScheme) {
            let
                ...
                pointsTMP = this._getPointsWithColValidators(validators);
            // console.log(configScheme);
            // console.log(configScheme["config"]["validators"]);
            for (const key in configScheme["colonnesResume"]) {
                if (!Object.hasOwn(configScheme["colonnesResume"], key)) continue;

                const nameCol = configScheme["colonnesResume"][key];
                for (let index = 0; index < pointsTMP.length; index++) {
                    const
                        indexColT = this.getColIndex(nameCol),
                        pt = pointsTMP[index];

                    points.push({ indexRow: pt.indexRow, indexCol: indexColT });
                }
            }

            points = pointsTMP.concat(points);
            let resume = this._iteratorAccumulator(points, configScheme["config"]["accumulator"]);
        ...
        // dataConfigXslSchemes = dataConfigXslSchemes.concat(dataConfigXslLocal);
        this.getResumeExportScheme = function (resumeMatrice, accumulators) {
            // console.log(resumeMatrice.getDataSet());

            for (let index = 0; index < resumeMatrice.getColonnes().length; index++) {
                const
                    colName = resumeMatrice.getColonnes()[index],
                    colIndex = resumeMatrice.getColIndex(colName),
                    accumul = accumulators[colName];

                // console.log(accumul);

                resumeMatrice.addColElement(resumeMatrice.getValSummeizeColWithAccumulator(colIndex, accumul), colName);
            }

            return [
                resumeMatrice.getColonnes(),
                resumeMatrice.getDataSet()
            ];
        }

        this.getConfigGroupExportSchemes = function (configSchemes, resumer, title = "Label Global Resume", recap = true) {
            let
                resumeMatrice = new tools.Library.Stats.Matrice(),
                accumulators = {},
                ws_data = [],
                resume = null,
                ws_data_title = [],
            ...
            for (const keyC in configSchemes) {
                if (Object.hasOwnProperty.call(configSchemes, keyC)) {
                    const
                        ...
                        accumulators_tmp = configS["config"]["accumulator"],
                        data = this.getConfigGroupExport(configS, configS["colonnesShow"], configS["title"]);
                    // data = this.getConfigGroupExport(configS["config"], configS["colonnesShow"], configS["title"]);
                    let
                        localResumeT = [configS["title"]],
                        localResumeH = data[1],
                        localResumeData = data[2],
                        localList = data.slice(4);
                    ...
                    // resume = resumer([localResumeData], resume);
                    for (let index = 0; index < localResumeH.length; index++) {
                        const head = localResumeH[index];
                        resumeMatrice.addColElement(localResumeData[index], head);
                        if (typeof accumulators_tmp == "function") {
                            accumulators[head] = accumulators_tmp;
                        } else {
                            if (accumulators[head] == null) {
                                accumulators[head] = resumer;
                            } else {
                                accumulators[head] = accumulators_tmp[head];
                            }
                        }
                    }
            ...
            resume = this.getResumeExportScheme(resumeMatrice, accumulators);
            console.log(resume);

            if (recap) {
                // ws_data_list.push(this.getResumeExportScheme(resumeMatrice, accumulators));
                ws_data.push(resume);
            }
        ...
        */
        this._iteratorAccumulator = function (indexColsRows, accumulators, direction = "Colonne") {
            ...
            for (const key in indexColsRows) {
                if (Object.hasOwnProperty.call(indexColsRows, key)) {
                    ...
                    if (typeof accumulators == "function") {
                        result[index] = accumulators(val, result[index]);

                    } else if (typeof accumulators == "object") {
        ...
        this.getValSummeizeColWithAccumulator = function (indexCol, accumulator) {
            let
                points = this._axePointsCollector("Colonnes", indexCol),
                summer = this._iteratorAccumulator(points, accumulator);
    ...
    MatriceUtilitary: {
        ...
        /**
 * mergeTrees()
 * Rôle : fusionner deux arbres issus de toTree(), quelle que
 *        soit leur structure d'origine (colonnes différentes,
 *        profondeurs différentes). Ne modifie ni treeA ni treeB
 *        — retourne un nouvel arbre fusionné.
 *
 * Règles de fusion (appliquées niveau par niveau) :
 *   1. Si une "value" (clé de children) existe dans les deux
 *      arbres au même niveau → on fusionne leurs rows (union
 *      sans doublon) et on fusionne récursivement leurs children.
 *   2. Si une "value" n'existe que dans un seul arbre → on la
 *      copie telle quelle dans le résultat.
 *
 * Paramètres :
 *   - treeA : premier arbre (objet retourné par toTree())
 *   - treeB : second arbre (objet retourné par toTree())
 *
 * Retour :
 *   Un nouvel arbre fusionné, de même forme que treeA / treeB.
 */
mergeTrees: function (treeA, treeB) {

            // Cas limites : un des deux arbres est absent
            if (!treeA) return tools.Library.Stats.MatriceUtilitary._cloneTree(treeB);
            if (!treeB) return tools.Library.Stats.MatriceUtilitary._cloneTree(treeA);

            // Fusionner les rows des deux nœuds (union sans doublon)
            let mergedRows = tools.Library.Stats.MatriceUtilitary._mergeRows(
                treeA.rows || [],
                treeB.rows || []
            );

            let mergedNode = {
                // On garde la value du nœud non-null en priorité
                // (les deux devraient être identiques si on fusionne
                //  au bon niveau, mais on sécurise quand même)
                value: treeA.value !== null && treeA.value !== undefined
                    ? treeA.value
                    : treeB.value,
                rows: mergedRows,
                children: {}
            ...
            let childrenA = treeA.children || {};
            let childrenB = treeB.children || {};

            // Récupérer toutes les clés présentes dans l'un OU l'autre
            let allKeys = {};
            for (let key in childrenA) {
                if (Object.hasOwnProperty.call(childrenA, key)) allKeys[key] = true;
            }
            for (let key in childrenB) {
                if (Object.hasOwnProperty.call(childrenB, key)) allKeys[key] = true;
            }

            // Pour chaque clé : fusionner si présente des deux côtés,
            // sinon copier le côté où elle existe
            for (let key in allKeys) {
                if (!Object.hasOwnProperty.call(allKeys, key)) continue;

                let inA = Object.hasOwnProperty.call(childrenA, key);
                let inB = Object.hasOwnProperty.call(childrenB, key);

                if (inA && inB) {
                    // Présent des deux côtés → fusion récursive (Règle 1)
                    mergedNode.children[key] = tools.Library.Stats.MatriceUtilitary.mergeTrees(
                        childrenA[key],
                        childrenB[key]
                    );
                } else if (inA) {
                    // Seulement dans A → copie telle quelle (Règle 2)
                    mergedNode.children[key] = tools.Library.Stats.MatriceUtilitary._cloneTree(childrenA[key]);
                } else {
                    // Seulement dans B → copie telle quelle (Règle 2)
                    mergedNode.children[key] = tools.Library.Stats.MatriceUtilitary._cloneTree(childrenB[key]);
                }
            }
        ...
        /**
         * _mergeRows()
         * Rôle : fonction interne — fusionne deux tableaux de noms
         *        de lignes en supprimant les doublons.
         */
        _mergeRows: function (rowsA, rowsB) {
            let result = rowsA.slice(); // copie de rowsA

            for (let i = 0; i < rowsB.length; i++) {
                if (result.indexOf(rowsB[i]) === -1) {
                    result.push(rowsB[i]);
                }
            }
        ...
        /**
         * _cloneTree()
         * Rôle : fonction interne — clone profondément un nœud d'arbre
         *        pour ne jamais modifier les arbres originaux passés
         *        en paramètre de mergeTrees().
         */
        _cloneTree: function (node) {
            ...
            let clone = {
                value: node.value,
                rows: node.rows ? node.rows.slice() : [],
                children: {}
            };
            ...
            for (let key in children) {
                if (Object.hasOwnProperty.call(children, key)) {
                    clone.children[key] = tools.Library.Stats.MatriceUtilitary._cloneTree(children[key]);
                }
            }
