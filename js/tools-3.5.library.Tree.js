...
(function() {
    ...
    tools.Library.Tree = function(name, value = null) {
         ...
         * ============================================================
         * 
         * Différence avec toJSON() ci-dessus :
         * toJSON() sert à l'AFFICHAGE / l'inspection (peut, selon l'appelant,
         * omettre les enfants), et est aussi le nom "magique" que
         * JSON.stringify() appelle automatiquement sur un objet.
         * toPlainObject() sert à la RECONSTRUCTION FIDÈLE de l'arbre
         * (round-trip complet via fromPlainObject) : toujours tous les
         * champs, toujours tous les enfants, jamais la référence "parent"
         * (qui créerait un cycle infini dans JSON.stringify).
         */

        /**
         * Convertit ce nœud (et récursivement tous ses enfants) en un objet
         * JavaScript "brut", sans référence circulaire vers `parent`,
         * prêt à être passé à JSON.stringify() puis restauré plus tard
         * avec tools.Library.Tree.fromPlainObject().
         * 
         * @returns {Object} Représentation sérialisable de l'arbre
         */
    ...
    /**
     * ============================================================
     * MÉTHODE STATIQUE : RECONSTRUCTION DEPUIS toPlainObject()
     * ============================================================
     * 
     * Reconstruit récursivement un arbre complet (this.children, this.parent,
     * this.depth, etc.) à partir d'un objet produit par toPlainObject().
     * 
     * @param {Object} obj - Objet brut produit par toPlainObject()
     * @param {tools.Library.Tree|null} [parent=null] - Le parent à assigner
     *        (utilisé en interne pour la récursion ; ne pas fournir à l'appel initial)
     * @returns {tools.Library.Tree} L'arbre reconstruit
     */
    tools.Library.Tree.fromPlainObject = function(obj, parent) {
        parent = parent || null;

        var node = new tools.Library.Tree(obj.name, obj.value);
        node.depth = obj.depth || 0;
        node.metadata = obj.metadata || {};
        node.impurityReduction = obj.impurityReduction || 0;
        node.featureName = (obj.featureName !== undefined) ? obj.featureName : null;
        node.samples = obj.samples || 0;
        node.parent = parent;

        var childrenObj = obj.children || [];
        node.children = childrenObj.map(function(childObj) {
            return tools.Library.Tree.fromPlainObject(childObj, node);
        });
